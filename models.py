...
class CNN2019(nn.Module):
    def __init__(self, in_channels, num_classes, avg_pool=1, conv_dim=32, hs=[64,32]):
        ...
        self.conv4 = conv(in_channels, conv_dim, 1,1,0)
        self.conv5 = conv(in_channels, conv_dim, 1,3,1)
        self.conv6 = conv(in_channels, conv_dim, 1,1,0)
        ...
        if len(hs) < 1:
            ...
            self.linears = [linear(conv_dim*avg_pool, hs[0])]
        else:
            self.linears = [linear(conv_dim*avg_pool, num)]
            for i, num in enumerate(hs[1:]):
                self.linears.append(linear(hs[i], num))
